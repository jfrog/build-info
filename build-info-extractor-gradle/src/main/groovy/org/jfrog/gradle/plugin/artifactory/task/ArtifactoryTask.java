...
public class ArtifactoryTask extends DefaultTask {
    ...
    public void projectEvaluated() {
        ...
        // When running the gradle build from a CI server or JFrog CLI with
        // the 'Project Uses the Artifactory Plugin' option set to false,
        // the init script generated by the CI server
        // sets the 'addArchivesConfigToTask' boolean to true,
        // so that the 'archives' gradle configuration is added.
        if (addArchivesConfigToTask) {
            helperConfigurations.AddDefaultArchiveConfiguration(getProject());
            addArchivesConfigToTask = false;
        }
        evaluated = true;
    }
