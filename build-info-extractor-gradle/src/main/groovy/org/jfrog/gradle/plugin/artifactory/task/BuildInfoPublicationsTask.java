...
public class BuildInfoPublicationsTask extends BuildInfoBaseTask {
  ...
  @InputFiles
  @Optional
  private Set<File> filesToPublish = Sets.newHashSet();
  ...
  private void addPublication(Publication publicationObj) {
    if (publicationObj instanceof IvyPublication) {
      ...
      IvyArtifactSet ivyArtifacts = ((IvyPublication) publicationObj).getArtifacts();
      for (IvyArtifact ivyArtifact : ivyArtifacts) {
        filesToPublish.add(ivyArtifact.getFile());
      }
    } else if (publicationObj instanceof MavenPublication) {
      mavenPublications.add((MavenPublication) publicationObj);

      MavenArtifactSet mavenArtifacts = ((MavenPublication) publicationObj).getArtifacts();
      for (MavenArtifact mavenArtifact : mavenArtifacts) {
        filesToPublish.add(mavenArtifact.getFile());
      }
    } else {
  ...
  public Set<File> getFilesToPublish() {
